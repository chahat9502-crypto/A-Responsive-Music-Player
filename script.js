const songs = [
  ...
  {
    title: "Namo Namo from Kedarnath",
    artist: "Amit Trivedi",
    duration: "5:22",
...
const mobileFavoriteCount = document.getElementById(
  "mobileFavoriteCount"
);
...
// Saved favorites and theme
let favorites =
  JSON.parse(localStorage.getItem("neonwaveFavorites")) || [];

const savedTheme =
  localStorage.getItem("neonwaveTheme") || "dark";
...
async function togglePlay() {
  if (audioPlayer.paused) {
    try {
      ...
      showToast(
        "Press play again if your browser blocked audio"
      );
    }
  } else {
    audioPlayer.pause();
...
function renderPlaylist() {
  const visibleSongs = songs
    ...
    .filter((song) => {
      return (
        currentFilter === "all" ||
        favorites.includes(song.originalIndex)
      );
    });
  ...
  if (visibleSongs.length === 0) {
    playlistElement.innerHTML = `
      <p class="empty-message">
        ...
        Tap the heart beside any song to save it here.
      </p>
    `;
  ...
  visibleSongs.forEach((song, listIndex) => {
    ...
    songRow.setAttribute(
      "aria-label",
      `Play ${song.title} by ${song.artist}`
    );

    songRow.style.setProperty(
      "--item-index",
      listIndex
    );
    ...
    const miniFavoriteButton =
      songRow.querySelector(".mini-favorite");
...
function updateFavoriteButtons() {
  ...
  mobileFavoriteCount.textContent = favorites.length;

  allCount.textContent = songs.length;
  mobileAllCount.textContent = songs.length;
}
...
audioPlayer.addEventListener("error", () => {
  ...
  showToast(
    "This audio could not load. Check the song file path."
  );
});
